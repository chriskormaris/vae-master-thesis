...
if __name__ == '__main__':
    ...
    for i in range(0, 38, 10):
        if i <= 20:
            fig = plot_dataset_samples.plot_yale_faces(X, y, categories=list(range(i, i + 10)),
                                 title='Original Faces', show_plot=False)
            fig.savefig(output_images_dir + '/original_faces_' + str(i+1) + '-' + str(i+10) + '.png')
            plt.close()
        else:
            fig = plot_dataset_samples.plot_yale_faces(X, y, categories=list(range(30, 38)),
                                  title='Original Faces', show_plot=False)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                fig = plot_dataset_samples.plot_yale_faces(cur_samples, batch_labels, categories=list(range(10)), title='Epoch {}'.format(str(epoch).zfill(3)))
