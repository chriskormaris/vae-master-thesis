...
def construct_missing_data(X, y=None, missing_value=0.5, structured_or_random='structured'):
    ...
    if structured_or_random == 'structured':
        ...
        if X.shape[1] == 784 or X.shape[1] == 1024:  # for MNIST, Binarized MNIST, OMNIGLOT and grayscaled CIFAR-10 datasets
            ...
            X_missing[int(X_missing.shape[0] / 5):int(X_missing.shape[0] * 2 / 5), :int(X_missing.shape[1] / 2), :] = missing_value

            # make the left half pixels missing
            X_missing[int(X_missing.shape[0] * 2 / 5):int(X_missing.shape[0] * 3 / 5), :, :int(X_missing.shape[2] / 2)] = missing_value

            # make the right half pixels missing
            X_missing[int(X_missing.shape[0] * 3 / 5):int(X_missing.shape[0] * 4 / 5), :, int(X_missing.shape[2] / 2):] = missing_value
