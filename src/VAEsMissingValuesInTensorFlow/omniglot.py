...
def omniglot(
    ...
    if language.lower() == 'greek':
        output_images_path = output_img_base_path + 'VAEsMissingValuesInTensorFlow/omniglot_greek'
        logdir = tensorflow_logs_path + 'omniglot_greek_vae_missing_values'
        save_path = save_base_path + 'omniglot_greek_vae_missing_values'
        alphabet = 20
    else:
        output_images_path = output_img_base_path + 'VAEsMissingValuesInTensorFlow/omniglot_english'
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch == 1:
                ...
                fig = plot_mnist_or_omniglot_data(masked_batch_data, batch_labels, categories=list(range(1, 11)),
                                                  title='Masked Data')
                fig.savefig(output_images_path + '/masked_data_characters_11-20.png'.format(str(epoch).zfill(3)),
                            bbox_inches='tight')
                plt.close()
                if language.lower() == 'greek':
                    fig = plot_mnist_or_omniglot_data(
